...
///
/// Shared by ``ChangelogItemRow`` and ``ChangelogItemDetailView`` so an entry's
/// icon looks identical at every size. Sizing scales with Dynamic Type through
/// the caller's `size`.
struct ChangelogItemIcon: View {
